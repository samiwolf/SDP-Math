...
class MyApp(Tk):
    def __init__(self):
        ...
        for idx, p_ in enumerate(self.particules):
            if idx == 0:
                ...
                self.particules[idx].gen(self.canvas.winfo_reqwidth(), self.canvas.winfo_reqheight(), 0, 0, 0, 0, 30,
                                         30)
                ...
                self.particules[idx].cv = self.canvas.create_oval(self.particules[idx].x - self.particules[idx].m,
                                                                  self.particules[idx].y - self.particules[idx].m,
                                                                  self.particules[idx].x + self.particules[idx].m,
                                                                  self.particules[idx].y + self.particules[idx].m)
                self.canvas.itemconfig(self.particules[idx].cv, fill=self.particules[idx].color)  # change color
            else:
                self.particules[idx] = particule()
                self.particules[idx].gen(self.canvas.winfo_reqwidth(), self.canvas.winfo_reqheight(), self.zmin,
                                         self.zmax, self.amin, self.amax, self.mmin, self.mmax)
                self.particules[idx].cv = self.canvas.create_oval(self.particules[idx].x - self.particules[idx].m,
                                                                  self.particules[idx].y - self.particules[idx].m,
                                                                  self.particules[idx].x + self.particules[idx].m,
                                                                  self.particules[idx].y + self.particules[idx].m)
    ...
    def update_drops(self):
        ...
        if len(self.particules) < 100:
            ...
            p = particule()
            p.gen(self.canvas.winfo_reqwidth(), self.canvas.winfo_reqheight(), self.zmin, self.zmax, self.amin,
                  self.amax, self.mmin, self.mmax)
            self.particules.append(p)
            self.particules[-1].cv = self.canvas.create_oval(self.particules[-1].x - self.particules[-1].m,
                                                             self.particules[-1].y - self.particules[-1].m,
                                                             self.particules[-1].x + self.particules[-1].m,
                                                             self.particules[-1].y + self.particules[-1].m)
            ...
            if idx > 0:
                for idy, py_ in enumerate(self.particules):
                    if idx != idy:
                        ...
                        self.particules[idx].v = [self.particules[idx].v[0] + self.particules[idx].a[0] * self.dt,
                                                  self.particules[idx].v[1] + self.particules[idx].a[1] * self.dt]

                        self.particules[idx].x += self.particules[idx].v[0] * self.dt
                        self.particules[idx].y += self.particules[idx].v[1] * self.dt

            self.canvas.coords(self.particules[idx].cv, self.particules[idx].x - self.particules[idx].m,
                               self.particules[idx].y - self.particules[idx].m,
                               self.particules[idx].x + self.particules[idx].m,
                               self.particules[idx].y + self.particules[idx].m)
