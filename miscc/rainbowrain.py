...
class MyApp(Tk):
    def __init__(self, totalDrops):
        ...
        for idx, drop_ in enumerate(self.drops):
            ...
            self.drops[idx].gen(self.canvas.winfo_reqwidth(), self.canvas.winfo_reqheight(), self.zmin, self.zmax,
                                self.speed, self.length, self.size)
            self.drops[idx].cv = self.canvas.create_line(self.drops[idx].x, self.drops[idx].y, self.drops[idx].x,
                                                         self.drops[idx].y + self.drops[idx].l, width=self.drops[idx].s)
    ...
    def update_drops(self):
        for idx, drop_ in enumerate(self.drops):
            ...
            self.canvas.coords(self.drops[idx].cv, self.drops[idx].x, self.drops[idx].y, self.drops[idx].x,
                               self.drops[idx].y + self.drops[idx].l)
