...
class LangtonsLoop(Tk):
    def __init__(self):
        ...
        self.Matpoint[self.h - P.shape[0] // 2:self.h + P.shape[0] // 2,
        self.w - P.shape[1] // 2:self.w + P.shape[1] // 2] = P
        ...
        for k in list(self.rules):
            if k[0] == 0:
                ...
                seven.append(k)
        self.rules = [np.array(zero), np.array(one), np.array(two), np.array(three), np.array(four), np.array(five),
                      np.array(six), np.array(seven)]

        for i in range(self.Matpoint.shape[0]):
            for j in range(self.Matpoint.shape[1]):
                self.circle[i][j] = self.canvas.create_oval((j - self.w // 2) * self.scale,
                                                            (i - self.h // 2) * self.scale,
                                                            (j - self.w // 2) * self.scale + self.scale,
                                                            (i - self.h // 2) * self.scale + self.scale)
    ...
    def update(self):
        ...
        for i in range(1, self.Matpoint.shape[0] - 1):
            for j in range(1, self.Matpoint.shape[1] - 1):
                ...
                neighbor1 = [self.Matpointcopy[i - 1, j], self.Matpointcopy[i, j + 1], self.Matpointcopy[i + 1, j],
                             self.Matpointcopy[i, j - 1]]
                neighbor2 = [self.Matpointcopy[i, j - 1], self.Matpointcopy[i - 1, j], self.Matpointcopy[i, j + 1],
                             self.Matpointcopy[i + 1, j]]
                neighbor3 = [self.Matpointcopy[i + 1, j], self.Matpointcopy[i, j - 1], self.Matpointcopy[i - 1, j],
                             self.Matpointcopy[i, j + 1]]
                neighbor4 = [self.Matpointcopy[i, j + 1], self.Matpointcopy[i + 1, j], self.Matpointcopy[i, j - 1],
                             self.Matpointcopy[i - 1, j]]
